...
class TranscodeVideo implements ShouldQueue
{
    ...
    public function handle () {        
        ...
        $media = $ffmpeg->open('/tmp/' . $this->filename);
        ...
        $media->save($format, '/tmp/' . $video_id . '.webm');
        $media->frame(FFMpeg\Coordinate\TimeCode::fromSeconds(12))->save('/tmp/' . $video_id . '.jpg');

        // Resize thumbnail
        Image::make('/tmp/' . $video_id . '.jpg')->fit(1280, 600, function($c) { $c->upsize(); })->save();

        // Push transcoded video and thumbnail to Google cloud
        Storage::disk('gcs_videos')->put($video_id . '.webm', file_get_contents('/tmp/' . $video_id . '.webm'), 'public');
        Storage::disk('gcs_videos')->put($video_id . '.jpg', file_get_contents('/tmp/' . $video_id . '.jpg'), 'public');

        // Apply local delete subsquently
        File::delete('/tmp/' . $this->filename);
        File::delete('/tmp/' . $video_id . '.webm');
        File::delete('/tmp/' . $video_id . '.jpg');
