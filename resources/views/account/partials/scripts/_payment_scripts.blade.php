...
<script>
    ...
    var datatable = $('.m_datatable').mDatatable({
        ...
        columns: [{
            ...
            template: function (response) {
                return '<a href="#" class="m-link m-link--info" onclick="event.preventDefault(); update_card(\'' + response.id + '\');">' + response.id.toUpperCase() + '</a>';
            }
        }, {
            field: "holder",
            title: "Cardholder",
            ...
            template: function (row) {
                ...
                return '\
                    <a href="#" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Edit" onclick="event.preventDefault(); update_card(\'' + row.id + '\');" data-skin="light" data-toggle="m-tooltip" data-placement="top" title="Editar">\
                        <i class="la la-edit"></i>\
                    </a>\
                    <a href="#" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Remove" onclick="event.preventDefault(); delete_card(\'' + row.id + '\');" data-skin="light" data-toggle="m-tooltip" data-placement="top" title="Remover">\
    ...
    function delete_card (token) {
        swal({
            ...
            cancelButtonText: 'No',
            confirmButtonClass: "btn btn-focus m-btn m-btn--air m-btn--wide",
            cancelButtonClass: "btn btn-secondary m-btn m-btn--wide",
        }).then(function(result) {
            if (result.value) {
                location.href = "{{ config('app.url') }}" + '/card/' + token + '/delete';
                swal(
                    'Deleted!',
